...
export const BillingPeriodsTable = <T extends object>({ }: ReactTableProps<T>) => {
                    ...
                    <Box display="flex" gap="1rem" >
                        <IconButton
                            onClick={(event) => {
                                ...
                                const billingPeriodId = row.row.original.id;
                                setBillingPeriodToEdit(billingPeriodId);
    ...
    return (
        <TableRenderer
            data={data?.data}
            columns={columns}
            onRowClick={(row: any) => router.push(`/billingperiods/${row._id}`)}
            pageInfo={data?.pageInfo}
        />
        // <TableRenderer
        //     data={data}
        //     columns={columns}
        //     onRowClick={(row: any) => router.push(`/billingsperiods/${row._id}`)}
        //     pageInfo={pageInfo}
        // />
    );
};
